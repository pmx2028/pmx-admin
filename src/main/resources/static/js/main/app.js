...
const App = function () {
    ...
    const navigationSidebar = function() {
        ...
        /************************************************************
            웹계산기
        ************************************************************/
        if ($(location).attr("pathname").indexOf("datacenter") > 0){
            $('.calculator').blackCalculator({type:'advanced', allowKeyboard:true, css:'/static/css/'});
        }
    ...
    const componentToTopButton = function() {
        ...
        if (scrollableContainer) {
            ...
            // /************************************************************
            //     오른쪽 top 버튼은 상세페이지에서만 노출되도록 수정
            // ************************************************************/
            // Show and hide on scroll
            const to_top_button = $('.btn-to-top'),
                add_class_on_scroll = function() {
                    if ($(location).attr("pathname").indexOf("form") > 0 || $(location).attr("pathname").indexOf("view") > 0 ){
